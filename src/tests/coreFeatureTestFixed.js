/**
 * Core Feature Testing - 24-Hour Tarot System & Spread System (FIXED)
...
runTest('Time Management - Functions', () => {
  ...
  const formatted12h = formatHour(14, false);
  const formatted24h = formatHour(14, true);
  const currentDate = getCurrentDate();
  const newDay1 = isNewDay('2020-01-01');
  const newDay2 = isNewDay(currentDate);
...
// Test: Spread card service (FIXED)
...
// Test: Spread store functionality (FIXED with proper async handling)
...
runTest('Spread Layout - Validation', () => {
  ...
  const invalidSpread1 = { spreadId: 'three_card', cards: [] };
  ...
  const invalidSpread3 = {
    ...
    cards: [
      { positionIndex: 0, cardKey: 'card_0' },
      { positionIndex: 1, cardKey: 'card_0' },
      { positionIndex: 2, cardKey: 'card_2' }
    ]
  };
...
async function runAllTests() {
  // Run the spread store test
  await runAsyncTest('Spread Store - State Management', testSpreadStore);
  ...
  const dailyTarotTests = 5;
  const spreadTests = 4;

  console.log('\n📋 FEATURE BREAKDOWN:');
  console.log(`🌅 24-Hour Tarot System: ${Math.min(dailyTarotTests, testResults.passed)}/${dailyTarotTests} tests passed`);
  console.log(`🃏 Spread System: ${Math.max(0, Math.min(spreadTests, testResults.passed - dailyTarotTests))}/${spreadTests} tests passed`);
